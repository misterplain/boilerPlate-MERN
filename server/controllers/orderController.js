...
const { sanitizeOrderFilters } = require("../utils/sanitize");
const {
  BadRequestError,
  ForbiddenError,
...
const getOrdersTimePeriod = asyncHandler(async (req, res) => {
  const { isAdmin } = req;
  ...
  const baseQuery = { datePlaced: { $gte: startPeriod } };
...
/**
 * Search orders with admin-controlled filter criteria.
 * Supports status, email, short id, postcode, and tracking number filters.
 */
const searchOrders = asyncHandler(async (req, res) => {
  const filterObject = sanitizeOrderFilters(req.body);
  if (!filterObject) {
...
/**
 * Place an authenticated user's order.
 * Performs stock deduction per cart item, creates an order, and clears user cart.
 */
